	...
	<el-dialog v-model="dialogFormVisible" :title="title" @closed="closeForm(ruleFormRef)">
		<el-form :model="form" label-width="120px" :rules="rules" ref="ruleFormRef"
			...
			<el-form-item label="角色" prop="role">
				<el-select v-model="form.role" multiple collapse-tags collapse-tags-tooltip placeholder="选择角色"
					size="large" style="width:100%" @change="handleChangeRole">
					<el-option v-for="v, k in userRoleListData" :key="k" :label="v['roleName']" :value="v['_id']" />
				</el-select>
			</el-form-item>
			...
			<el-form-item label="备注" prop="remarks">
				<el-input v-model="form.remarks" :rows="4" type="textarea" placeholder="请输入备注" />
			</el-form-item>
		</el-form>
		<template #footer>
...
//依赖
import { AdminUserControllerUpdate, AdminUserControllerUpdatePassword, AdminUserControllerCreate } from '@/api/AdminUserControllerApi';
import type { AdminUserDto } from '@/api/dto/AdminUserDto';
import { RolePermissionsControllerGetList, RolePermissionsControllerGetPage } from '@/api/RolePermissionsControllerApi';
	...
	await formEl.validate(async (valid, fields) => {
		if (valid) {
			...
			if (addUser.value) {
				...
				const { data: res } = await AdminUserControllerUpdate(updateId.value, form.value);
				data = res;
			}
			if (data.status === 1) {
