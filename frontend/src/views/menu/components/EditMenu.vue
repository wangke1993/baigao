    ...
    <el-dialog v-model="dialogFormVisible" :title="title" @closed="closeForm(ruleFormRef)">
        <el-form :model="form" label-width="120px" :rules="rules" ref="ruleFormRef"
            ...
            <el-form-item label="属性" prop="menuType">
                <el-select v-model="form.menuType" placeholder="选择属性" size="large" style="width:100%">
                    <el-option v-for="v, k in menuTypeSelect" :key="k" :label="v" :value="k" />
                </el-select>
            </el-form-item>
            <el-form-item v-show="Number(form.menuType) === 1" label="是否显示" prop="isShow">
                <el-switch v-model="form.isShow" active-value="1" inactive-value="0" />
            </el-form-item>
            <el-form-item v-show="Number(form.menuType) === 1 && form.parentDeep.length === 0" label="图标"
                prop="menuIcon">
                <el-input placeholder="element-plus ICON 名称" v-model="form.menuIcon" />
            </el-form-item>
            <el-form-item v-show="Number(form.menuType) === 1" label="路由" prop="menuActive">
                <el-input v-model="form.menuActive" />
            </el-form-item>
            <el-form-item v-show="Number(form.menuType) > 1" label="权限标识" prop="menuPowerTag">
                <el-input v-model="form.menuPowerTag" />
            </el-form-item>
            <el-form-item v-show="Number(form.menuType) === 1" label="排序" prop="sort">
...
//依赖
import { AdminMenuControllerCreate, AdminMenuControllerUpdate } from '@/api/AdminMenuControllerApi';
import type { AdminMenuDto } from '@/api/dto/AdminMenuDto';
import { MENU_TYPE_TRANSLATE, MENU_TYPE_TRANSLATE_NO_MENU } from '@/utils/enum/menuType';
...
const open = (row: any, add: boolean = true) => {
    ...
    if (add) {
        if (row) {
            title.value = '添加子级';
            if (row.parentDeep.length >= 2) { //最大可以添加3级菜单
    ...
    await formEl.validate(async (valid, fields) => {
        if (valid) {
            ...
            if (addMenu.value) {
                ...
                const { data: res } = await AdminMenuControllerUpdate(updateId.value, form.value);
                data = res;
            }
            if (data.status === 1) {
