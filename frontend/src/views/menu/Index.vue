<template>
    <div class="list-box" v-loading="load">
        <el-button type="primary" @click="add(null)" v-if="btnShow('menu_add_top')">添加一级菜单</el-button>
        <el-table :data="menuTree" row-key="_id" empty-text="无数据">
            ...
            <el-table-column prop="menuType" label="属性">
                <template #default="scope">
                    <div>
                        <span :style="{ color: MENU_TYPE_TRANSLATE_COLOR[scope.row.menuType] }">
            ...
            <el-table-column label="操作">
                <template #default="scope">
                    <el-button-group class="ml-4">
                        <el-button v-if="btnShow('menu_edit')" @click="edit(scope.row)" type="primary" :icon="Edit">
                            编辑
                        </el-button>
                        <el-button v-if="btnShow('menu_add_child') && scope.row.menuType === 1" @click="add(scope.row)"
                            type="primary" :icon="Plus">
                            添加子级
                        </el-button>
                        <el-popconfirm v-if="btnShow('menu_del')" confirm-button-text="确定" cancel-button-text="取消"
...
// 依赖
import { AdminMenuControllerDelete, AdminMenuControllerGetTree } from '@/api/AdminMenuControllerApi';
import { alertSuccess, alertWarning } from '@/utils/message';
import { ref } from 'vue';
import { MENU_TYPE_TRANSLATE, MENU_TYPE_TRANSLATE_COLOR } from '@/utils/enum/menuType'
import {
    Delete,
    Edit,
    Plus,
