    ...
    <el-table :data="logListData" row-key="_id" v-loading="load">
      ...
      <el-table-column prop="operationIp" label="操作IP" :width="150">
        <template #default="scope">
          <a
            :href="`https://ip.tool.chinaz.com/${scope.row.operationIp}`"
            target="_blank"
          >
            {{ scope.row.operationIp }}
          </a>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作时间" :width="200">
        <template #default="scope">
          {{
            DateTime.fromISO(scope.row.operationTime, {
              zone: "utc+8",
...
import {
  FileUploadControllerUpload,
  FileUploadControllerGetPrivateFile,
} from "@/api/FileUploadControllerApi";
