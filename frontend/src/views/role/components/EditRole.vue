...
const rules = reactive<FormRules>({
  ...
  permissionsList: [{ required: true, message: "不能为空", trigger: "change" }],
  indexPath: [],
  isSuper: [{ required: true, message: "不能为空", trigger: "blur" }],
});
  ...
  await formEl.validate(async (valid, fields) => {
    ...
    if (valid) {
      ...
      if (addUser.value) {
        ...
        const { data: res } = await RolePermissionsControllerUpdate(
          updateId.value,
          form.value
        );
        data = res;
