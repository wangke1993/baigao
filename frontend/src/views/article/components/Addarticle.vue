<template>
  <el-dialog v-model="dialogFormVisible" :title="title" :close-on-press-escape="false" :close-on-click-modal="false"
    width="70%" @closed="closeForm(ruleFormRef)">
    <el-form :model="form" label-width="120px" :rules="rules" ref="ruleFormRef"
      ...
      <el-form-item prop="cover" label="文章封面">
        <div>
          <el-upload v-model:file-list="fileList" list-type="picture-card" action="/api/file/upload" name="file"
            :auto-upload="true" :limit="1" :on-success="onSuccess">
            <el-icon>
              <Plus />
            </el-icon>
      ...
      <el-form-item label="分类" prop="articleClass">
        <el-select v-model="form.articleClass" class="m-2" placeholder="请选择文章分类" size="large" style="width: 100%">
          <el-option v-for="(item, index) in dictionaryData" :key="index" :label="item.name" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="概要" prop="syn">
        <el-input v-model="form.syn" type="textarea" :rows="4" placeholder="请输入文章概要" />
      </el-form-item>
      <el-form-item label="是否发布" prop="release">
        <el-switch v-model="form.release" />
      </el-form-item>
      <el-form-item label="有效期" prop="expirationDate">
        <el-date-picker v-model="form.expirationDate" type="datetime" placeholder="请选择有效期" />
      </el-form-item>
...
import { ArticleMangerControllerCreate, ArticleMangerControllerGetDetailByArticleId, ArticleMangerControllerUpdate } from "@/api/ArticleMangerControllerApi";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // 编辑文章
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const editArticle = async (id: string, parms: any) => {
      try {
        let result = await ArticleMangerControllerUpdate(id, parms);
        let data = result.data;
        if (data.status === 1) {
    ...
    // 发布文章接口
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const postArticleMangerControllerCreate = async (parms: any) => {
      try {
    ...
    editorConfig.MENU_CONF["uploadVideo"] = { ...editorConfig.MENU_CONF["uploadImage"] };
