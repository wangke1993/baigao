<template>
  <el-form v-if="state.open" :model="form" label-width="120px" :rules="rules" ref="ruleFormRef"
    ...
    <el-form-item>
      <el-button v-loading="state.loading" type="primary" @click="submitForm(ruleFormRef)">保存</el-button>
    </el-form-item>
  </el-form>
</template>
...
import { TreeClassificationControllerCreate, TreeClassificationControllerUpdate } from "@/api/TreeClassificationControllerApi";
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      ...
      if (id.value) {
        // 更新
        const { data: res } = await TreeClassificationControllerUpdate(id.value, form.value)
        if (res.status == 1) {
          alertSuccess("成功")
        } else {
        ...
        const { data: res } = await TreeClassificationControllerCreate(form.value)
        if (res.status == 1) {
          alertSuccess("成功")
        } else {
          alertWarning(res.message)
