<template>
	<el-form :inline="true" :model="searchForm" ref="searchFormRef" class="demo-form-inline">
		...
		<el-form-item label="文章分类：">
			<el-select v-model="searchForm.articleClass" class="m-2" placeholder="请选择文章分类">
				<el-option v-for="(item,index) in articleClass" :key="index" :label="item.dicName" :value="item.dicCode" />
			</el-select>
		</el-form-item>
		<el-form-item>
...
const getArticleDictionary = async (dicCode: string) => {
	try {
		...
		if (data.status === 1) articleClass.value = [...articleClass.value, ...data.data];
	} catch (err) {
		return;
	}
};
